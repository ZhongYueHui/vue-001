...
2. 点击加载更多，让 page++ , 然后重新调用 this.getComments() 方法重新获取最新一页的数据
...
## 首页和商品详情页的图片样式问题
1. 首页的轮播图宽度100%,但是商品详情页的100%会很丑,我们可以让他自适应,所以需要在轮播图组件中带一个是否需要宽度100%的参数 `isfull`给 `props`,并且在轮播图的img身上加上`:class="{'full': isfull}"`,这样我们在使用轮播图的组件的时候可以通过给组件加上`isfull`,例如:`<swpie :swpieList="swpieList" :isfull="true"></swpie>`,这样我们就可以通过传递值来决定是否需要宽度为100%了
